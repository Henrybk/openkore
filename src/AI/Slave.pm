...
sub processFollow {
	...
	if (
		...
		if (!$field->canMove($slave->{pos_to}, $char->{pos_to})) {
			$slave->route(undef, @{$char->{pos_to}}{qw(x y)}, noMapRoute => 1, avoidWalls => 0, randomFactor => 0, isFollow => 1);
...
sub processIdleWalk {
	...
	if (
		...
		# Random square
			...
			$slave->route(undef, @{$walk_pos}{qw(x y)}, attackOnRoute => 2, noMapRoute => 1, avoidWalls => 0, randomFactor => 0, isIdleWalk => 1);
