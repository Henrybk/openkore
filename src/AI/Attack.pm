...
sub main {
	...
	if ($char->{sitting}) {
		...
		if ($cell) {
			...
			$args->{avoiding} = 1;
			$char->route(undef, @{$cell}{qw(x y)}, noMapRoute => 1, avoidWalls => 0, randomFactor => 0, runFromTarget => 1);
		} else {
		...
		if ($best_spot) {
			...
			$char->route(undef, @{$best_spot}{qw(x y)}, noMapRoute => 1, avoidWalls => 0, randomFactor => 0, LOSSubRoute => 1);
		} else {
