...
sub cmdTestPing {
	$messageSender->sendGetPlayerInfo(pack("V", 2000000)); # to be used with pingGMpp [OK]
}
...
my $commands_hooks = Commands::register(
	...
	['testping', 'test ping',			\&cmdTestPing],
);
...
my $myHooks = Plugins::addHooks(
	...
	['packet_areaSpell',						\&detectGM_analyseSkillCaster],
...
my %server_name_lut = (
	'51.81.56.97:6900' => 'Oath',
	'localhost:6900' => 'testoff',
);
...
my $ping_idArrayPosition = 0; # should start as 0.
my $ping_last_time = 0;
sub AIpre_pingCheck {
	return unless ($core_config{ping_GM});
	my $size = scalar @{$core_databases{GMIDS}};
	return unless ($size);
	...
	my $count = 1;
	while ($count <= $core_config{ping_IDperLoop}) {
		my $id = $core_databases{GMIDS}[$ping_idArrayPosition];
		debug "[koreShield] [AIpre_pingCheck] pinging GM index [$ping_idArrayPosition] id [$id]\n";
		if ($ping_idArrayPosition == $#{$core_databases{GMIDS}}) {
			$ping_idArrayPosition = 0;
		} else {
			$ping_idArrayPosition++;
		}
		$messageSender->sendGetPlayerInfo(pack("V", $id));
	} continue {
		$count++;
	}
	$ping_last_time = time;
...
sub detectGM_analyseSkillCaster {
	...
	} elsif ($caller eq 'packet_areaSpell') {
		if ($args->{sourceID} && (isIn_Array(unpack("V1", $args->{sourceID}), \@{$core_databases{GMIDS}}))) {
			error sprintf("Player de ID %s da blacklist usou %s! Desconectando... \n", unpack("V1", $args->{sourceID}), $skillname), "koreShield_detect";
			&core_eventsReaction('blacklisted_used_skill');
			return;
